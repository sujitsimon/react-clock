...
export default function CountDownTimer(props) {
    ...
    const isCountDownTimeEnd = useRef(false);
    ...
    useEffect(()=> {
        let currentTimeout = setTimeout(() => {
            ...
            if (state.countDownDate) {
                let timer = new Date(state.countDownDate - date);
                let hours = timer.getUTCHours();
                let minutes = timer.getUTCMinutes();
                let seconds = timer.getUTCSeconds();
                if (!isCountDownTimeEnd.current) setState(solve_time(hours, minutes, seconds));
                if (hours === 0 && minutes === 0 && seconds === 0 && !isCountDownTimeEnd.current) {
                    clearTimeout(currentTimeout);
                    isCountDownTimeEnd.current = true;
    ...
    useEffect(() => {
        if(props.countDownTime){
            ...
            countDownDate.setHours(countDownDate.getHours() + props.countDownTime.hours);
            countDownDate.setMinutes(countDownDate.getMinutes() + props.countDownTime.minutes);
            countDownDate.setSeconds(countDownDate.getSeconds() + props.countDownTime.seconds);
            setState({...state, countDownDate: countDownDate});
        }
